...
const PropertyCard: React.FC<PropertyCardProps> = ({
  ...
  return (
    <a
      href={`/listings?property=${encodeURIComponent(property.id)}`}
      onClick={event => {
...
export default function ListingsPage({ properties }: ListingsPageProps) {
  ...
  const canonicalUrl = selectedProperty
    ? `${siteOrigin}/listings?property=${encodeURIComponent(selectedProperty.id)}`
    : `${siteOrigin}/listings`;
  ...
  const structuredData = useMemo(() => {
    const itemList = activeProperties.slice(0, 50).map((property, index) => ({
      ...
      position: index + 1,
      name: property.title,
      url: `${siteOrigin}/listings?property=${encodeURIComponent(property.id)}`
    }));
      ...
      },
      {
        ...
        mainEntity: [
          ...
          },
          {
            ...
            acceptedAnswer: {
              '@type': 'Answer',
              text: 'Có. Khách hàng có thể gửi khoảng ngân sách, khu vực và loại hình mong muốn để được lọc giỏ hàng phù hợp.'
            }
          }
        ]
      }
          ...
          <div className="mx-auto grid max-w-7xl gap-10 px-4 lg:grid-cols-[1.1fr_0.9fr]">
            <article>
              ...
              <p className="mt-5 leading-7 text-slate-600">
                Giỏ hàng Estoria tập trung các loại hình căn hộ, nhà phố, đất và shophouse tại Đà Nẵng.
