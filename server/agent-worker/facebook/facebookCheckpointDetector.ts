...
export function isNonRetryableBrowserErrorMessage(message: string): boolean {
  return /SOURCE_REMOVED|FACEBOOK_LOGIN_REQUIRED|FACEBOOK_CHECKPOINT|FACEBOOK_CHALLENGE|FACEBOOK_SESSION_UNKNOWN|FACEBOOK_MODAL_STUCK|FACEBOOK_WRONG_SCROLL_TARGET|FACEBOOK_FEED_NOT_RECOVERED|FACEBOOK_NAVIGATION_STATE_UNKNOWN|CDP_UNREACHABLE|CDP_NO_CONTEXT|BROWSER_PROFILE_LOCKED/.test(
    message,
  );
}
