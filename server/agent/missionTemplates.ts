/**
 * Built-in AI Employee mission templates (Sprint 7.1).
 * Templates are catalog data — not DB rows until the user creates a mission from one.
 */

export interface MissionTemplateSchedule {
  /** Human-readable cadence for CMS; scheduler still source-driven in 6.1 */
  cadence: 'hourly' | 'every_2h' | 'every_4h' | 'daily' | 'manual';
  preferredHoursLocal?: number[];
  timezone?: string;
}

export interface MissionTemplateRules {
  sourceIds: string[];
  positiveKeywords: string[];
  negativeKeywords: string[];
  minFindingScore: number;
  /** Alias used by findingRuleEngine */
  minScore: number;
  notifyScore: number;
  maxItemsPerRun: number;
  analysisInstructions: string;
  /** Preferred source types when sourceIds empty — UI hint only */
...
export interface MissionTemplate {
  ...
  name: string;
  objective: string;
  rules: MissionTemplateRules;
  schedule: MissionTemplateSchedule;
}
...
export const MISSION_TEMPLATES: MissionTemplate[] = [
  {
    ...
    schedule: {
      cadence: 'every_4h',
      preferredHoursLocal: [8, 12, 16, 20],
      timezone: TZ,
    },
  },
  {
    ...
    objective:
      'Theo dõi nhu cầu thuê mặt bằng kinh doanh / văn phòng / shop tại Đà Nẵng để kết nối nguồn hàng thuê.',
    rules: {
      sourceIds: [],
      preferredSourceTypes: ['facebook_group', 'website', 'forum'],
      positiveKeywords: [
    ...
    schedule: {
      cadence: 'every_2h',
      preferredHoursLocal: [7, 9, 11, 14, 17, 20],
      timezone: TZ,
    },
  },
  {
    ...
    objective:
      'Theo dõi tín hiệu mua/thuê căn hộ (chung cư) để phát hiện lead và xu hướng nhu cầu theo khu vực.',
    rules: {
      sourceIds: [],
  ...
  },
  {
    ...
    objective:
      'Quét các Facebook group đã cấu hình để phát hiện chủ đề / nhu cầu đang nóng trong ngày (không chỉ lead bán hàng).',
    rules: {
      sourceIds: [],
    ...
    schedule: {
      cadence: 'hourly',
      preferredHoursLocal: [7, 8, 9, 10, 11, 12, 14, 16, 18, 20],
      timezone: TZ,
    },
  },
  {
    ...
    objective:
      'Định kỳ đọc các website/forum BĐS đã gắn vào mission để thu thập tin mới và finding theo rule.',
    rules: {
      sourceIds: [],
    ...
    schedule: {
      cadence: 'every_4h',
      preferredHoursLocal: [6, 10, 14, 18, 22],
      timezone: TZ,
    },
  },
];
...
export function buildMissionPayloadFromTemplate(
  ...
  const sourceIds = overrides?.sourceIds?.length
    ? overrides.sourceIds
    : template.rules.sourceIds;
