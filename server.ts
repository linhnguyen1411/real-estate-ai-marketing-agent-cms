...
function formatPublicPropertySuggestions(properties: Property[]) {
  if (!properties.length) {
    return 'Hiện giỏ hàng chưa có sản phẩm công khai phù hợp để em gửi anh/chị.';
  }
  ...
  return properties.map((property, index) => {
    ...
    return [
      ...
      `- 🔗 [**Xem chi tiết sản phẩm**](/listings?property=${encodeURIComponent(property.id)})`
    ].filter(Boolean).join('\n');
  }).join('\n\n');
}
  ...
  try {
    ...
    if (directProperty && (asksPrice || asksLegal || asksArea || asksLocation)) {
      ...
      const directResponse = [
        ...
        `- ${details.split(', ').join('\n- ')}`,
        `- 🔗 [**Xem chi tiết sản phẩm**](/listings?property=${encodeURIComponent(directProperty.id)})`,
    ...
    let aiResponse = await generateText(
      [
        ...
        'Mỗi sản phẩm bắt buộc có link xem chi tiết theo đúng id: [Xem chi tiết sản phẩm](/listings?property=<id>).',
        ...
        '- ✨ **Điểm đáng chú ý:** ...',
        '- **Phù hợp khi:** ...',
        '- 🔗 [**Xem chi tiết sản phẩm**](/listings?property=<id sản phẩm>)',
        '### 2. Tên sản phẩm',
        '... cùng cấu trúc ...',
    ...
    if (
      ...
      && (!/###\s+\d+\.|(?:^|\n)-\s+/m.test(aiResponse) || !aiResponse.includes('/listings?property='))
    ) {
      aiResponse = [
    ...
    if (!aiResponse.includes('/listings?property=')) {
      ...
      if (mentionedProperties.length) {
        aiResponse = [
          ...
          ...mentionedProperties.map(property =>
            `- [${property.title}](/listings?property=${encodeURIComponent(property.id)})`
          )
        ].join('\n\n');
      }
    }
...
app.get('/sitemap.xml', (req: Request, res: Response) => {
  ...
  const urls = [
    `${origin}/listings`,
    ...publicProperties.map(property => `${origin}/listings?property=${encodeURIComponent(property.id)}`)
  ];
  const xml = [
