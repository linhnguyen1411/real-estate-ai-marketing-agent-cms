...
```text
├── /backend            # Mã nguồn tham khảo Sinatra API (Ruby)
│   └── app.rb          # Sinatra API chính điều chuyển database db.json và Ollama
...
### 3. Chạy Backend bằng Ruby (Sinatra API tham khảo)
...
3. Chạy ứng dụng Sinatra:
   ...
   ruby app.rb
   ```
Máy chủ backend Ruby của bạn sẽ tự động khởi động tại địa chỉ `http://localhost:4567`. Bạn có thể tinh chỉnh API fetch của frontend sang cổng `4567` này trong mã nguồn khi triển khai.
