...
Module crawler ch?y ngay trong MVP ? ch? ngu?n public, kh�ng bypass ??ng nh?p/captcha/t�i kho?n c� nh�n.

### Ngu?n h? tr?
- Google Search theo keyword (n?u b? ch?n ? ghi log, kh�ng bypass)
- Website HTML public (`start_url`)
- RSS/Atom feed
- Li�n k?t public t? trang g?c
...
### Frontend
`/admin/crawler-jobs` ? qu?n l� job, n�t "Ch?y ngay", b?ng k?t qu? v� log.

### Scheduler
Express + **node-cron** ? t? ch?y job `active` theo interval.

### Lead extractor
Ph�n lo?i intent: `buy` | `sell` | `rent` | `lease` | `unknown` ? l?u CRM (`source: crawler`), ch?ng tr�ng theo `phone + source_url`.
